...
public class EnderbiteCocoonBlock extends Block {
	private static final VoxelShape ONE_EGG_AABB = Block.box(3.0D, 0.0D, 3.0D, 12.0D, 7.0D, 12.0D);

	public EnderbiteCocoonBlock(BlockBehaviour.Properties properties) {
		super(properties);
	}
	...
	private void decreaseEggs(Level p_57792_, BlockPos p_57793_, BlockState p_57794_) {
		p_57792_.playSound((Player) null, p_57793_, SoundEvents.WOOL_BREAK, SoundSource.BLOCKS, 0.7F, 0.9F + p_57792_.random.nextFloat() * 0.2F);
	...
	public void randomTick(BlockState p_57804_, ServerLevel p_57805_, BlockPos p_57806_, Random p_57807_) {
		if (this.shouldUpdateHatchLevel(p_57805_)) {
			p_57805_.playSound((Player) null, p_57806_, SoundEvents.WOOL_BREAK, SoundSource.BLOCKS, 0.7F, 0.9F + p_57807_.nextFloat() * 0.2F);
			p_57805_.removeBlock(p_57806_, false);
			p_57805_.levelEvent(2001, p_57806_, Block.getId(p_57804_));
			Enderbite enderbite = ModEntities.ENDERBITE.get().create(p_57805_);
			enderbite.moveTo((double) p_57806_.getX() + 0.5D, (double) p_57806_.getY(), (double) p_57806_.getZ() + 0.5D, 0.0F, 0.0F);
			p_57805_.addFreshEntity(enderbite);
		}
	}
	...
	private boolean shouldUpdateHatchLevel(Level p_57766_) {
		return p_57766_.random.nextInt(5) == 0;
	}
